...
# --- Patch ws.c: fix conflicting types for curl_ws_meta ---
# ws.c uses internal `struct Curl_easy *` but the public header declares `CURL *`.
# Both are the same type but the compiler sees them as different without the typedef.
# Add the typedef at the top of ws.c to resolve the conflict.
ws_path = os.path.join(curl_src, 'ws.c')
if os.path.exists(ws_path):
    raw = open(ws_path, 'rb').read()
    s = raw.replace(b'\r\n', b'\n').decode('utf-8')
    # Add typedef at top of file to make struct Curl_easy == CURL
    fix = """/* Vitra iOS: ensure CURL typedef is visible before ws.c function definitions */
#ifndef CURL_DEFINED
...
system_h_path = os.path.join(
    os.path.dirname(os.path.dirname(config_path)),  # _deps/curl-build/
